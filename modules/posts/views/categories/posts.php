...
<div class="clearfix" ></div>
		...
		<section class="content">
			...
			<div class="content__item" id="<?='content-'.$j?>">
                            <?= Html::img('@web/images/imac-content.jpg',['class' => 'content__item-img rounded-right','alt'=>'iMac Content']);?>
				<div class="content__item-inner">
					<h2><?=$category->title?></h2>
					<h3><?=$post->title?></h3>
					<?= Html::tag('p', $post->discription, ['class' => 'categories_discription'])?>
					<p><?= Html::a('Оставить отзыв', ['categories/single-post','id'=>$post->id_posts], ['class' => 'btn btn-lg btn-success']) ?></p>
				</div>
			</div>
                    <?php $j++?>
                    <?php endforeach;?>
